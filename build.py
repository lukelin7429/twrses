...
# ---------------- Dom Jones · HTML slide decks (115 學年度起) ----------------
# 114 學年度的簡報是預先算圖的 JPG；115 起改為 HTML 投影片，才能在流程裡嵌動畫、
# 讓題目點一下就揭曉答案（JPG 版一題要吃掉「出題」「解答」兩頁）。投影片固定畫在
# 1200x675 的畫布上，由 deck.js 依舞台寬度等比縮放，不依賴新的 CSS 特性。
HTML_DECK_DIR = os.path.join(ROOT, "data", "decks")

def _load_html_decks():
    decks = []
    if not os.path.isdir(HTML_DECK_DIR):
        return decks
    for fn in sorted(os.listdir(HTML_DECK_DIR)):
        if fn.endswith(".json"):
            decks.append(json.load(open(os.path.join(HTML_DECK_DIR, fn), encoding="utf-8")))
    return decks

HTML_DECKS = _load_html_decks()
...
def _hs_title(s, d):
    chips = "".join(_bi(html.escape(c["en"]), html.escape(c["zh"]), "hs-chip") for c in s.get("chips", []))
    strip = "".join(
        f'<div class="hs-stat"><b>{html.escape(x["num"])}</b>'
        f'<span class="hs-en">{html.escape(x["en"])}</span>'
        f'<span class="hs-zh">{html.escape(x["zh"])}</span></div>'
        for x in s.get("strip", []))
    return f'''<div class="hs hs-title">
  <div class="hs-titlerow">
    <div class="hs-badge"><span>SDG</span><b>{html.escape(d["sdg"])}</b></div>
    <div class="hs-titletext">
      <h2>{html.escape(d["title_en"])}</h2>
      <p class="hs-zh-big">{html.escape(d["title"])}</p>
    </div>
  </div>
  <div class="hs-rule"></div>
  {_bi(html.escape(s["eyebrow_en"]), html.escape(s["eyebrow_zh"]), "hs-eyebrow")}
  <div class="hs-chips">{chips}</div>
  <div class="hs-strip">{strip}</div>
  <div class="hs-school">{html.escape(d["school"])} {html.escape(d["school_en"])} · {html.escape(d["date"])}</div>
</div>'''

def _hs_intro(s, d):
    lines = "".join(_bi(html.escape(x["en"]), html.escape(x["zh"]), "hs-line") for x in s["lines"])
    return f'''<div class="hs hs-intro">
  <h3 class="hs-head">{html.escape(s["head_en"])}<span>{html.escape(s["head_zh"])}</span></h3>
  <div class="hs-lines">{lines}</div>
  <div class="hs-ask">🙋 {_bi(html.escape(s["ask_en"]), html.escape(s["ask_zh"]))}</div>
</div>'''

def _hs_video(s, d):
    return f'''<div class="hs hs-video">
  <h3 class="hs-head">{html.escape(s["head_en"])}<span>{html.escape(s["head_zh"])}</span></h3>
  <div class="hs-yt" data-yt="{html.escape(s["yt"])}">
    <img src="https://i.ytimg.com/vi/{html.escape(s["yt"])}/hqdefault.jpg" alt="">
    <button type="button" class="hs-play" aria-label="播放動畫 Play animation">▶</button>
  </div>
  <div class="hs-note">{_bi(html.escape(s["note_en"]), html.escape(s["note_zh"]))}</div>
</div>'''

def _hs_standup(s, d):
    items = "".join(
        f'<li>{_bi(html.escape(x["en"]), html.escape(x["zh"]))}</li>' for x in s["prompts"])
    return f'''<div class="hs hs-standup">
  <h3 class="hs-head">🙋 {html.escape(s["head_en"])}<span>{html.escape(s["head_zh"])}</span></h3>
  <ol class="hs-prompts">{items}</ol>
  <div class="hs-punch">{_bi(html.escape(s["punch_en"]), html.escape(s["punch_zh"]))}</div>
</div>'''

def _hs_topic(s, d):
    kws = "".join(
        f'<span class="hs-kw"><b>{html.escape(k["en"])}</b>{html.escape(k["zh"])}</span>'
        for k in s.get("keywords", []))
    return f'''<div class="hs hs-topic">
  <h3 class="hs-head">{html.escape(s["head_en"])}<span>{html.escape(s["head_zh"])}</span></h3>
  <p class="hs-body-en">{html.escape(s["body_en"])}</p>
  <p class="hs-body-zh">{html.escape(s["body_zh"])}</p>
...
def _hs_quiz(s, d):
    letters = "ABCD"
    opts = "".join(
        f'<button type="button" class="hs-opt{" is-right" if i == s["answer"] else ""}" data-opt="{i}">'
        f'<span class="hs-letter">{letters[i]}</span>'
        f'<span class="hs-en">{html.escape(o["en"])}</span>'
        f'<span class="hs-zh">{html.escape(o["zh"])}</span></button>'
  ...
  <div class="hs-qbar">
    <span class="hs-qtag">🎮 Game Time! 遊戲時間</span>{bonus}
    <span class="hs-qn">Q{s["n"]}</span>
  </div>
  <div class="hs-q">{_bi(html.escape(s["q_en"]), html.escape(s["q_zh"]))}</div>
  <div class="hs-opts">{opts}</div>
  <div class="hs-explain">✅ {_bi(html.escape(s["explain_en"]), html.escape(s["explain_zh"]))}</div>
  <div class="hs-tap">👆 點任一選項公布答案 · Tap any option to reveal</div>
</div>'''

def _hs_bigstat(s, d):
    return f'''<div class="hs hs-bigstat">
  <div class="hs-bignum">{html.escape(s["num"])}</div>
  <h3 class="hs-bighead">{html.escape(s["head_en"])}</h3>
  <p class="hs-bighead-zh">{html.escape(s["head_zh"])}</p>
  <div class="hs-rule"></div>
  <div class="hs-lines">{_bi(html.escape(s["body_en"]), html.escape(s["body_zh"]), "hs-line")}</div>
  <div class="hs-ask">❓ {_bi(html.escape(s["ask_en"]), html.escape(s["ask_zh"]))}</div>
</div>'''

def _hs_truefalse(s, d):
    items = "".join(
        f'<button type="button" class="hs-tf" data-answer="{"T" if x["answer"] else "F"}">'
        f'<span class="hs-tfq">{_bi(html.escape(x["en"]), html.escape(x["zh"]))}</span>'
        f'<span class="hs-tfa">{"⭕ TRUE" if x["answer"] else "❌ FALSE"}'
        f'<em>{html.escape(x["note_en"])}<br>{html.escape(x["note_zh"])}</em></span></button>'
        for x in s["items"])
    return f'''<div class="hs hs-tflist">
  <h3 class="hs-head">{html.escape(s["head_en"])}<span>{html.escape(s["head_zh"])}</span></h3>
  <div class="hs-tfs">{items}</div>
  <div class="hs-tap">👆 點每一題公布答案 · Tap each one to reveal</div>
</div>'''

def _hs_game(s, d):
    hints = "".join(f'<span class="hs-hint">{html.escape(h)}</span>' for h in s.get("hints", []))
    bonus = '<span class="hs-bonus">BONUS 加碼</span>' if s.get("bonus") else ""
    return f'''<div class="hs hs-game">
  <h3 class="hs-head">{html.escape(s.get("icon", "🎯"))} {html.escape(s["head_en"])}
    <span>{html.escape(s["head_zh"])}</span></h3>{bonus}
  <div class="hs-rule-box">{_bi(html.escape(s["rule_en"]), html.escape(s["rule_zh"]))}</div>
  <div class="hs-hints">{hints}</div>
  <div class="hs-target">{_bi(html.escape(s["target_en"]), html.escape(s["target_zh"]))}</div>
</div>'''

def _hs_pairshare(s, d):
    return f'''<div class="hs hs-pair">
  <h3 class="hs-head">👥 {html.escape(s["head_en"])}<span>{html.escape(s["head_zh"])}</span></h3>
  <div class="hs-prompt">{_bi(html.escape(s["prompt_en"]), html.escape(s["prompt_zh"]))}</div>
  <div class="hs-sentence"><b>{html.escape(s["sentence_en"])}</b><span>{html.escape(s["sentence_zh"])}</span></div>
  <div class="hs-time">⏱ {_bi(html.escape(s["time_en"]), html.escape(s["time_zh"]))}</div>
</div>'''

def _hs_rank(s, d):
    items = "".join(
        f'<div class="hs-rankitem"><b>{i+1}</b>{_bi(html.escape(x["en"]), html.escape(x["zh"]))}</div>'
        for i, x in enumerate(s["items"]))
    return f'''<div class="hs hs-rank" data-reveal-group>
  <h3 class="hs-head">✋ {html.escape(s["head_en"])}<span>{html.escape(s["head_zh"])}</span></h3>
  <div class="hs-sub">{_bi(html.escape(s["sub_en"]), html.escape(s["sub_zh"]))}</div>
  <div class="hs-ranks">{items}</div>
  <button type="button" class="hs-revealbtn">公布 Reveal</button>
  <div class="hs-explain">{_bi(html.escape(s["reveal_en"]), html.escape(s["reveal_zh"]))}</div>
</div>'''

def _hs_action(s, d):
    items = "".join(
        f'<div class="hs-act"><b>{html.escape(x["num"])}</b>'
        f'{_bi(html.escape(x["en"]), html.escape(x["zh"]))}</div>' for x in s["items"])
...
def _hs_phrases(s, d):
    items = "".join(
        f'<div class="hs-phrase"><b>{html.escape(x["en"])}</b><span>{html.escape(x["zh"])}</span></div>'
        for x in s["items"])
...
def _qr_svg_path(slug, url):
    """在本機產生 QR 的 SVG 存進 repo——教室網路擋掉外部服務也不會開天窗。"""
    import qrcode, qrcode.image.svg
    rel = f"media/dom-jones/slides/{slug}/qr.svg"
...
def _hs_padlet(s, d):
    url = d.get("padlet", "")
    qr = _qr_svg_path(d["slug"], url)
    return f'''<div class="hs hs-padlet">
  <h3 class="hs-head">📱 {html.escape(s["head_en"])}<span>{html.escape(s["head_zh"])}</span></h3>
  <div class="hs-padletrow">
    <img class="hs-qr" src="{html.escape(qr)}" alt="Padlet QR code" loading="lazy">
    <div>
      <div class="hs-prompt">{_bi(html.escape(s["body_en"]), html.escape(s["body_zh"]))}</div>
      <div class="hs-sentence"><b>{html.escape(s["sentence_en"])}</b>
        <span>{html.escape(s["sentence_zh"])}</span></div>
    </div>
  </div>
</div>'''
...
def _hs_closing(s, d):
  ...
  <div class="hs-bigline-zh">{html.escape(s["big_zh"])}</div>
  <div class="hs-rule"></div>
  <div class="hs-sign">{html.escape(s["sign_en"])}<span>{html.escape(s["sign_zh"])}</span></div>
</div>'''

_HS_RENDER = {
    "title": _hs_title, "intro": _hs_intro, "video": _hs_video, "standup": _hs_standup,
    "topic": _hs_topic, "quiz": _hs_quiz, "bigstat": _hs_bigstat, "truefalse": _hs_truefalse,
    "game": _hs_game, "pairshare": _hs_pairshare, "rank": _hs_rank, "action": _hs_action,
    "phrases": _hs_phrases, "padlet": _hs_padlet, "closing": _hs_closing,
}

def _html_deck_body(d, back_href):
    slides = []
    for s in d["slides"]:
        fn = _HS_RENDER.get(s["type"])
        if not fn:
            raise SystemExit(f"unknown slide type {s['type']!r} in {d['slug']}")
        slides.append(f'<div class="deck-slide hs-wrap"><div class="hs-canvas" data-theme="{d.get("theme","amber")}">'
                      f'{fn(s, d)}<div class="hs-foot">{html.escape(d["footer"])}</div></div></div>')
    n = len(slides)
    return f'''
<div class="deck-page">
  <div class="deck">
    <div class="deck-top">
      <a class="deck-back" href="{back_href}">← 簡報庫 Slide Library</a>
      <div class="deck-heading"><h1>{html.escape(d["title"])}</h1>
        <div class="sub">{html.escape(d["school"])} · {html.escape(d["date"])}</div></div>
...
{chr(10).join(slides)}
      ...
      <button class="deck-arrow prev" aria-label="上一頁 Previous">&lsaquo;</button>
      <button class="deck-arrow next" aria-label="下一頁 Next">&rsaquo;</button>
    </div>
    <div class="deck-progress"><div class="bar"></div></div>
    <p class="deck-hint">👉 向右滑動看下一頁 · 方向鍵 ← → 也可切換 · 題目點一下公布答案</p>
  </div>
</div>
<script src="/assets/js/deck.js?v={ASSET_V}"></script>'''
...
def build_html_decks():
    ...
    for d in HTML_DECKS:
        body = _html_deck_body(d, back)
        path = f"/media/dom-jones/slides/{d['slug']}/"
        write(path, layout(path, f"{d['title']} · {d['school']} 簡報",
              f"Dom Jones 到{d['school']}（{d['date']}）的雙語互動簡報。", body, "media"))
...
def build_dom_jones_slides():
  ...
  <span class="deck-body"><h4>{html.escape(title)}</h4><span class="sub">{html.escape(sub)}</span><span class="cnt">{count} 頁 →</span></span>
    ...
    def html_card(d):
        """115 起的 HTML 簡報沒有 01.jpg 可當封面，改用 CSS 畫的色塊封面。"""
        return f'''<a class="deck-card deck-card-html rvl" href="/media/dom-jones/slides/{d["slug"]}/" id="deck-{d["slug"]}">
  <span class="deck-cover deck-cover-html" data-theme="{d.get("theme","amber")}">
    <span class="dc-sdg">SDG {html.escape(d["sdg"])}</span>
    <span class="dc-title">{html.escape(d["title_en"])}</span>
    <span class="dc-zh">{html.escape(d["title"])}</span>
  </span>
  <span class="deck-body"><h4>{html.escape(d["title"])}</h4><span class="sub">{html.escape(d["school"])} · {html.escape(d["date"])}</span><span class="cnt">{len(d["slides"])} 頁 · 可互動 →</span></span>
</a>'''
    ...
    y115 = [d for d in HTML_DECKS if str(d.get("year")) == "115"]
    y115_html = ""
    if y115:
        y115_cards = "\n".join(html_card(d) for d in y115)
        y115_html = ('<div class="deck-lib-group">\n'
                     '    <h3>115 學年度（2026–2027）· 互動簡報 Interactive Decks</h3>\n'
                     f'    <div class="grid cols-3 stagger">{y115_cards}</div>\n'
                     '  </div>')
...
<section class="section"><div class="wrap">
  ...
  <div class="deck-lib-group">
    <h3>114 學年度（2025–2026）· School Visit Decks</h3>
...
def main():
    ...
    build_html_decks()
    for _d in HTML_DECKS:
        paths.append(f"/media/dom-jones/slides/{_d['slug']}/")
