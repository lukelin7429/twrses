...
(function () {
  ...
  /* 🔊 buttons — a recorded clip where we have one, the device voice otherwise.
     ...
     served from a per-page GitHub release (release assets cap out at 1000, and
     a single page never comes close); the manifest maps a phrase to its filename.
  ...
  (function loadSayManifest() {
    ...
    if (!slug) return;
    sayBase = 'https://github.com/lukelin7429/twrses/releases/download/say-' + slug + '/';
  ...
  window.say = function (text, btn) {
    ...
    if (!src) {
      ...
      if (!hash || !sayBase) { sayFallback(text, btn); return; }
      src = sayBase + hash + '.mp3';
    }
